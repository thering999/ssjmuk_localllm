...
import { 
  ...
  Clock, BarChart3, FileText, Copy, Pin, PinOff,
  Settings2, Eye
} from 'lucide-react'
...
export default function PlaygroundPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      updateCurrentSession([...updatedMessages, { id: Date.now().toString(), role: 'assistant', content: `⚠️ SYSTEM ERROR: ${err.message}`, timestamp: Date.now() }])
    } finally {
      setLoading(false)
    }
  }
  ...
  const exportChat = () => {
    const text = messages.map(m => `[${m.role.toUpperCase()}]\n${typeof m.content === 'string' ? m.content : 'Binary Attached'}\n`).join('\n---\n\n')
          ...
          <motion.aside initial={{ width: 0, opacity: 0 }} animate={{ width: 280, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="flex flex-col gap-3 shrink-0 overflow-hidden border-r border-white/5 pr-2">
            ...
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-4">
               <div className="flex justify-between items-center">
                  <span className="text-[9px] font-black uppercase text-muted-foreground tracking-widest">Aura Theme</span>
      ...
      <div className={`flex-1 flex flex-col rounded-[2.5rem] border-2 border-white/20 bg-black/20 backdrop-blur-[100px] overflow-hidden min-h-0 shadow-3xl relative transition-all duration-500`}>
                  ...
                  <div className={`relative flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} max-w-[85%]`}>
                    <div className={`rounded-2xl px-5 py-3 text-[13.5px] leading-relaxed shadow-xl border backdrop-blur-3xl transition-all ${msg.role === 'user' ? `bg-gradient-to-br ${THEMES[activeTheme]} text-white border-white/10 rounded-tr-none shadow-primary/20` : 'bg-white/95 dark:bg-zinc-900/95 text-foreground border-white dark:border-zinc-800 rounded-tl-none ai-bubble'}`}>
                       <div className="markdown-content font-medium tracking-tight prose prose-sm prose-emerald dark:prose-invert max-w-none">
                          <ReactMarkdown remarkPlugins={[remarkGfm]}>{typeof msg.content === 'string' ? msg.content : 'Multi-modal data link.'}</ReactMarkdown>
                       </div>
                    </div>
                    <div className="flex items-center gap-4 mt-1.5 px-3 transition-all opacity-0 group-hover/msg:opacity-100">
                       <span className="text-[8px] font-black text-white/20 uppercase">{new Date(msg.timestamp).toLocaleTimeString()}</span>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 flex items-center gap-3">
                 ...
                 <span className="text-[10px] font-black uppercase text-primary tracking-widest">Neural Syncing</span>
              </div>
            </div>
          )}
        ...
        <div className="p-4 bg-white/5 border-t border-white/10 relative z-20 backdrop-blur-3xl">
          ...
          <div className="mt-3 flex items-center justify-center gap-6 opacity-20">
             ...
             <p className="text-[7px] font-black uppercase tracking-[0.5em] text-white whitespace-nowrap">Intelligence System Protocol v5.0 · Compact Matrix</p>
             <div className="h-px flex-1 bg-white/10" />
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showWorkspace && (
          <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 480, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="flex flex-col gap-3 shrink-0 overflow-hidden">
             <div className="flex-1 rounded-[2rem] bg-white/5 dark:bg-black/20 backdrop-blur-[100px] border border-white/10 p-5 shadow-3xl relative flex flex-col">
                <header className="flex items-center justify-between mb-4">
                   ...
                   <Button variant="ghost" size="sm" onClick={() => { const blob = new Blob([workspaceContent], { type: 'text/plain' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'PHO-Neural-Draft.txt'; a.click(); }} className="h-6 text-[8px] bg-white/10 text-white uppercase font-black px-4 rounded-lg border border-white/10 hover:bg-white/20 transition-all">Download</Button>
                </header>
                ...
                <textarea 
                  ...
                  placeholder="Drafting stream initialized..."
                />
             ...
             <div className="p-6 rounded-[2rem] bg-white/5 border border-white/10 space-y-6 shadow-2xl">
                <div className="flex justify-between items-center">
                   ...
                   <Badge variant="outline" className="h-10 px-4 rounded-xl border-emerald-500/20 bg-emerald-500/5 text-emerald-400 flex flex-col items-center justify-center">
                      <span className="text-[12px] font-black tabular-nums">฿{(totalTokens * 0.000035).toFixed(4)}</span>
                      <span className="text-[7px] font-bold uppercase leading-none mt-1 tracking-widest">Saved</span>
                   </Badge>
                </div>
                <div className="space-y-5 pt-5 border-t border-white/5">
                   ...
                   <div className="space-y-2">
                      ...
                      <Select value={selectedModel} onValueChange={(v) => setSelectedModel(v ?? 'auto')}>
                         ...
                         <SelectContent className="rounded-2xl border-white/10 backdrop-blur-3xl bg-zinc-950/95 shadow-3xl">
