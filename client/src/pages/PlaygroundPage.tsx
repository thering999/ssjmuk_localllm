  ...
  return (
    <div className={`flex h-[calc(100vh-8rem)] max-w-[1750px] mx-auto w-full group/main gap-3 relative animate-in fade-in duration-500 theme-${activeTheme}`}>
      ...
      <AnimatePresence>{rightPanel !== 'none' && (<motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 450, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="flex flex-col gap-3 shrink-0 overflow-hidden">
             ...
             {rightPanel === 'workspace' && (<div className="flex-1 rounded-[3rem] bg-white/5 dark:bg-black/40 backdrop-blur-[100px] border border-white/15 p-8 shadow-[0_0_80px_rgba(0,0,0,0.5)] relative flex flex-col h-full animate-in zoom-in-95 duration-300"><header className="flex items-center justify-between mb-6"><div className="flex items-center gap-3"><div className="size-12 rounded-2xl bg-secondary/10 flex items-center justify-center text-secondary border border-secondary/20 shadow-glow-sm"><FileText className="size-6" /></div><span className="text-[12px] font-black uppercase text-white/60 tracking-[0.3em] leading-none">Drafting Unit</span></div><Button variant="ghost" size="sm" onClick={() => { const blob = new Blob([workspaceContent], { type: 'text/plain' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'Analysis-Draft.txt'; a.click(); }} className="h-6 text-[8px] bg-white/10 text-white uppercase font-black px-4 rounded-lg border border-white/10 hover:bg-white/20 transition-all">Download</Button></header><Separator className="bg-white/5 mb-6" /><textarea value={workspaceContent} onChange={(e) => setWorkspaceContent(e.target.value)} className="flex-1 w-full bg-transparent text-[15px] font-medium text-white/90 focus:text-white focus:outline-none resize-none leading-relaxed custom-scrollbar prose prose-sm dark:prose-invert placeholder:opacity-5" placeholder="Drafting matrix initialized..." /><div className="mt-4 flex justify-between items-center opacity-30 text-[8px] font-black uppercase tracking-tighter"><span>{workspaceContent.length} CHARS | {workspaceContent.split(/\s+/).length} WORDS</span><span>Sync Active</span></div></div>)}
